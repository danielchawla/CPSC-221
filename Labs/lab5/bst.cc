     ...
     */
     if (parent != NULL){ // if target != root
      ...
      if(root == target){ 
        Node* pre = root->left;
        while(pre->right) {
        	pre = pre->right; // find predecessor
        }
        root->key = pre->key;
        delete_node(root->left, pre->key); 
        target = NULL;
      } else return false;
     } 
  }
...
int numLeaves( Node* root ) {
  ...
  int num = 0;
  if(root){
    if(!root->left && !root->right) num++;
    else {
      num+= numLeaves(root->left);
      num+= numLeaves(root->right); 
    }
  }
  return num;
