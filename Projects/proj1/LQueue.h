...
/*-- LQueue.h -------------------------------------------------------------
  ...
  Basic operations:
    ...
    move_to_front:      Search through queue for particular value. If found, move node to front of same queue.
    merge_two_queues:   Merges two ordered queues into one ordered queue.
...
void move_to_front(const QueueElement & key);
  /*-----------------------------------------------------------------------
    Search through queue for particular key value. If found, move node 
		to front of same queue.

    Precondition:  Queue is nonempty.
    Postcondition: If node is found, node is moved to front of same
				array. 
  -----------------------------------------------------------------------*/

void merge_two_queues( Queue &queue2);
  /*-----------------------------------------------------------------------
    Takes two queues that are each ordered according to a given field. 
		Merges queues into one queue whose absolute order of the merged 
		elements is kept.

    Precondition:  Both queues are empty and ordered.
    Postcondition: All elements in q2 are added to queue in such a way that queue 
		remains ordered. 
  -----------------------------------------------------------------------*/
